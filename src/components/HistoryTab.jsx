...
// Import ikon Lucide
import { History, Droplet, Sprout, ThermometerSun, Loader2, RefreshCw } from 'lucide-react';
      ...
      for (const deviceId of devices) {
        ...
        snapshot.forEach((doc) => {
          ...
          allLogs.push({
            ...
            status: data.moisture >= 80 ? 'watered' : data.moisture < 30 ? 'thirsty' : 'normal'
          });
        });
      }
    ...
    <div className="flex-1 min-h-0 w-full px-6 pt-4 pb-24 overflow-y-auto animate-fadeIn no-scrollbar relative z-10">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center h-40 text-white/80 gap-3">
          ...
          <p className="font-bold text-sm tracking-wider uppercase">Memuat Catatan...</p>
        </div>
      ) : logs.length === 0 ? (
        // Tampilan Kosong
        <div className="bg-white/20 backdrop-blur-md rounded-[2rem] p-8 text-center border border-white/30 shadow-sm flex flex-col items-center">
          ...
          <h3 className="text-white font-bold text-lg mb-1">Belum Ada Aktivitas</h3>
          <p className="text-white/70 text-xs font-medium">Siram pot Anda untuk melihat riwayatnya di sini.</p>
        </div>
      ) : (
        ...
        <div className="relative border-l-2 border-white/30 ml-4 pl-6 space-y-6">
          {logs.map((log, idx) => (
            <div key={log.id + idx} className="relative">
              ...
              <div className={`absolute -left-[35px] top-2 w-4 h-4 rounded-full border-4 border-[#85C4FF] shadow-sm
              ...
              <div className="bg-white/20 backdrop-blur-md rounded-3xl p-4 flex items-center gap-4 border border-white/30 shadow-sm hover:bg-white/30 transition-colors">
                ...
                <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 shadow-inner
                  ${log.status === 'watered' ? 'bg-blue-100 text-blue-500' : 
                    log.status === 'thirsty' ? 'bg-red-100 text-red-500' : 
                    'bg-green-100 text-green-500'}
                `}>
                  {log.status === 'watered' ? <Droplet size={24} fill="currentColor" /> : 
                   log.status === 'thirsty' ? <ThermometerSun size={24} /> : 
                   <Sprout size={24} />}
                </div>

                <div className="flex-1">
                  <h3 className="text-white font-black text-lg leading-tight">
                    {log.status === 'watered' ? 'Disiram' : log.status === 'thirsty' ? 'Terlalu Kering' : 'Kondisi Aman'}
                  </h3>
